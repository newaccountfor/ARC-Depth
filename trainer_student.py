...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.use_teacher == True:
            ...
            for model_type in ["encoder", "depth"]:
                ...
                if model_type == "encoder":
                    pretrained_dict_for_student = {k: v for k, v in pretrained_dict.items() if k in self.student_help_teacher_encoder}
                    self.student_help_teacher_encoder.update(pretrained_dict_for_student)
                    for param in self.student_help_teacher_encoder.parameters():
                        param.requires_grad = False
                    self.student_help_teacher_encoder.to(self.device)
                else:
                    pretrained_dict_for_student = {k: v for k, v in pretrained_dict.items() if k in self.student_help_teacher_decoder}
                    self.student_help_teacher_decoder.update(pretrained_dict_for_student)
            ...
            self.teacher_encoder = networks.ResnetEncoder(50, None)
            self.teacher_decoder = networks.TeacherDecoder()
            for model_type in ["teacher_encoder", "teacher_decoder"]:
                model_path = os.path.join(self.opt.teacher_model_path, model_type)
                pretrained_dict = torch.load(model_path)
                if model_type == "teacher_encoder":
                    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in self.teacher_encoder}
                    self.teacher_encoder.update(pretrained_dict)
                    for param in self.teacher_encoder.parameters():
                        param.requires_grad = False
                    self.teacher_encoder.to(self.device)
                else:
                    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in self.teacher_decoder}
                    self.teacher_decoder.update(pretrained_dict)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            if self.opt.use_teacher == True:
                ...
                # teacher outputs
                teacher_input = torch.cat((source1, disp1_help_teacher, source0, disp0_help_teacher, source2, disp2_help_teacher), 1)
        ...
        if self.opt.reconstruction_idea == True:
            self.generate_features_pred(outputs)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.use_teacher == True:
                disp_teacher = outputs[("disp_teacher", disp)]
    ...
    def generate_features_pred(self, outputs):
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            disp = outputs[("disp", scale)]
            if self.opt.use_teacher == True:
                disp_teacher = outputs[("teacher_disp", scale)]
            color = inputs[("color", 0, scale)]
            target = inputs[("color", 0, source_scale)]

            # teacher min loss
            if self.opt.use_teacher == True:
                teacher_min_loss = 0 # will be filled later
                student_min_loss = 0 # will be filled later
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                if self.opt.use_teacher == True:
                    teacher_pred = outputs[("color", frame_id, scale)]
                ...
                if self.opt.pose_idea == True:
                    ...
                    student_min_loss = final
                    if self.opt.use_teacher == True:
                        teacher_min_loss = torch.min(torch.cat((teacher_loss_pred, teacher_loss_pred_for_t, teacher_loss_pred_r_and_t, teacher_loss_pred_for_r), 1), 1, True)
                else:
                    final = loss_pred
                    student_min_loss = final
                    if self.opt.use_teacher == True:
                        teacher_min_loss = loss_pred
            ...
            if self.opt.reconstruction_idea == True:
                ...
                min_perceptional_loss = torch.min(perceptional_loss, dim=1)
