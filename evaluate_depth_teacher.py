...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        print("-> Loading weights from {}".format(opt.load_weights_folder_teacher))
        ...
        # teacher network
        encoder_teacher_path = os.path.join(opt.load_weights_folder_teacher, "encoder_t.pth")
        decoder_teacher_path = os.path.join(opt.load_weights_folder_teacher, "depth_t.pth")
        ...
        with torch.no_grad():
            ...
            for data in dataloader:
                ...
                if torch.cuda.is_available():
                    ...
                    input_color_1 = data[("color", -1, 0)].cuda()
                    input_color_0 = data[("color", 0, 0)].cuda()
                    input_color_2 = data[("color", 1, 0)].cuda()
                
                else:
                    input_color_1 = data[("color", -1, 0)].cpu()
                    input_color_0 = data[("color", 0, 0)].cpu()
                    input_color_2 = data[("color", 1, 0)].cpu()

                if opt.post_process:
                    # Post-processed results require each image to have two forward passes
                    input_color = torch.cat((input_color, torch.flip(input_color, [3])), 0)

                disp1_help_teacher = depth_decoder(encoder(input_color_1))
                disp0_help_teacher = depth_decoder(encoder(input_color_0))
                disp2_help_teacher = depth_decoder(encoder(input_color_2))


                teacher_input = torch.cat((input_color_1, disp1_help_teacher[("disp", 0)], input_color_0, disp0_help_teacher[("disp", 0)], input_color_2, disp2_help_teacher[("disp", 0)]), 1)
                output = decoder_teacher(encoder_teacher(teacher_input))

                pred_disp_0, _ = disp_to_depth(output[("disp_t", 0)], opt.min_depth, opt.max_depth)
